...
# ********************************************************************************************************
# wget -q "--no-check-certificate" https://github.com/biko-73/Feed/raw/main/openpli-8.3.sh -O - | /bin/sh
# ********************************************************************************************************
...
#Check image name 
imgnm=$(curl -s $url1 | grep -o 'href="[^"]*\.zip"' | grep "8.3" | awk -F'"' '{print $2; exit}' | sed 's/^.*openpli-/openpli-/')
