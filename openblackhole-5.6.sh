...
# ***************************************************************************************************************
# wget -q "--no-check-certificate" https://github.com/biko-73/Feed/raw/main/openblackhole-5.6.sh -O - | /bin/sh
# ***************************************************************************************************************
...
sleep 7s
imgnm=$(curl -s  "https://images.openbh.net/?b=5.6%2F$brand%2F$device" | grep -o 'href="[^"]*\.zip"' | awk -F'"' '{print $2}'| sed 's/^.*openbh/openbh/' | sed '/recovery/d')
...
wget -O $ms/images/openbh-5.6-$device-"$today"_multi.zip $url
fi
