            ...
            if os.path.exists(rating_file):
                with open(rating_file, "r") as f:
                    ...
                    try:
                        ...
                        tn_log(f"[STARS] تحويل التقييم العددي: {rating} → {self.value}")
                    except ValueError:
                        stars = RATING_TO_STARS.get(content, 0)
                        self.value = stars
                        tn_log(f"[STARS] تحويل التقييم النصي: {content} → {stars}")
            else:
