...
class TN_X_Downloader(threading.Thread):
    ...
    def download_all(self, item):
        ...
        if self.lng == "ar":
            search_langs = ["ar", "en"] + [l for l in langs if l not in ["ar", "en"]]
        else:
            search_langs = ["en", self.lng] + [l for l in langs if l not in ["en", self.lng]]

        tn_log(f"[LANG] ترتيب البحث: {search_langs}")

        for lang in search_langs:
            result = self.search_tmdb(title, desc, lang, clean_name, poster_file, backdrop_file, logo_file, banner_file, rating_file, cast_file)
            if result:
    ...
    def search_tmdb(self, title, desc, lang, clean_name, poster_file, backdrop_file, logo_file, banner_file, rating_file, cast_file):
        try:
            ...
            for res in data["results"]:
                ...
                tn_log(f"[RATING] تم استخراج التصنيف: {rating}")
                with open(rating_file, "w", encoding="utf-8") as f:
                    f.write(rating)

                # --- استخدام fanart.tv ---
                self.fetch_from_fanart(tmdb_id, media_type, logo_file, banner_file)
    ...
    def fetch_from_fanart(self, tmdb_id, media_type, logo_file, banner_file):
        try:
            url = f"https://webservice.fanart.tv/v3/{'movies' if media_type == 'movie' else 'tv'}/{tmdb_id}?api_key={fanart_api}"
            ...
            # --- لوجو من fanart.tv ---
            hdlogos = data.get("hdmovielogo" if media_type == "movie" else "hd-tvlogo", [])
            if hdlogos and not os.path.exists(logo_file):
                self.download_image(hdlogos[0]["url"], logo_file)

            # --- بانر من fanart.tv ---
            banners = data.get("moviebanner" if media_type == "movie" else "tvbanner", [])
            if banners and not os.path.exists(banner_file):
                self.download_image(banners[0]["url"], banner_file)
