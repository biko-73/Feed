...
# دعم TMDb, Fanart.tv, TheTVDB, Google + إدارة ذاكرة تلقائية
# 2025
...
# --- دالة: التحقق من المساحة المتاحة (بالميغابايت) ---
...
# --- دالة: البحث عن أفضل وسيلة تخزين ---
def get_external_storage():
    ...
    for path in ['/media/hdd', '/media/usb', '/tmp']:
        if is_writable(path):
            ...
            if free_mb > 500:  # فقط إذا كانت المساحة > 500MB
                candidates.append((path, free_mb))

    if candidates:
        # الأفضل: الأكبر مساحة
        best = max(candidates, key=lambda x: x[1])
        print(f"[PosterX] Using storage: {best[0]} (Free: {best[1]:.1f} MB)")
        return best[0]

    # إذا لم يتوفر مكان كافٍ، استخدم /tmp كحل أخير
    if os.access('/tmp', os.W_OK):
        free_tmp = get_free_space('/tmp')
        if free_tmp > 100:
            print(f"[PosterX] Low space! Using /tmp (Free: {free_tmp:.1f} MB)")
            return '/tmp'

    print("[PosterX] No writable storage with sufficient space!")
    return None

# --- إنشاء هيكل المجلدات ---
def create_poster_folders(base_path):
    main_folder = os.path.join(base_path, "TNPoster_X")
    folders = {
    ...
    # إنشاء ملف معلومات
    info_file = os.path.join(main_folder, "storage_info.txt")
    with open(info_file, "w") as f:
        f.write(f"Storage Path: {base_path}\n")
        f.write(f"Main Folder: TNPoster_X\n")
        f.write(f"Created on: {time.ctime()}\n")
        f.write(f"Free Space: {get_free_space(base_path):.1f} MB\n")
    
    return folders

# --- تحديد أفضل وسيلة تخزين ---
base_storage = get_external_storage()
if not base_storage:
    base_storage = "/tmp"  # كحل أخير قسري

# --- إنشاء الهيكل ---
FOLDERS = create_poster_folders(base_storage)

# --- روابط المجلدات ---
path_folder = FOLDERS["poster"]        # للبوستر الرئيسي
backdrop_folder = FOLDERS["backdrop"]  # للخلفيات
banner_folder = FOLDERS["banner"]      # للبنرات
logo_folder = FOLDERS["logo"]          # للشعارات
