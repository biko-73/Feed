...
class TN_X_Downloader(threading.Thread):
    ...
    def download_all(self, item):
        ...
        recent_files = [
            f for f in [poster_file, backdrop_file, logo_file, rating_file, cast_file]
            if os.path.exists(f) and time.time() - os.path.getmtime(f) < 86400
        ]
        if recent_files:
            tn_log(f"[CACHE] ملفات حديثة - تخطي: {clean_name}")
    ...
    def download_image(self, url, filename):
        try:
            tn_log(f"[IMAGE] تنزيل من: {url} إلى {filename}")
            ...
            if r.status_code == 200:
                ...
                tn_log(f"[IMAGE] فشل HTTP: {r.status_code}")
        except Exception as e:
