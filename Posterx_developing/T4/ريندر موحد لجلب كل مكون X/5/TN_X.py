...
class TN_X(Renderer):
    ...
    def changed(self, what):
        ...
        try:
            ...
            # 1. حاول من self.source.event (الحدث الحالي)
            if hasattr(self.source, "event"):
                event = self.source.event

            # 2. حاول من getCurrentEvent (مثل session.Event_Now)
            elif hasattr(self.source, "getCurrentEvent"):
                event = self.source.getCurrentEvent()

            # 3. دعم خاص لـ Event[2], Event[3], ... في الشاشة
            elif hasattr(self.source, "getEvent") and hasattr(self.source, "getCurrentService"):
                try:
                    # استخراج الرقم من الـ source
                    source_name = str(self.source).split()[-1]  # مثل: 'Event[2]'
                    if "[" in source_name and "]" in source_name:
                        index = int(source_name.split("[")[1].split("]")[0])
                        service = self.source.getCurrentService()
                        event = self.source.getEvent(index)
                except Exception as e:
                    tn_log(f"[EVENT_INDEX] خطأ في استخراج الحدث: {e}")

            # 4. حاول من func() (مثل Event[1])
            elif hasattr(self.source, "func"):
                try:
                    event = self.source.func()
                except:
                    pass
