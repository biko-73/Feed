...
# تعديل: استخدم اسم مجلدك (مثل GOLD_DRAGON_FHD)
curskin = "GOLD_DRAGON_FHD"
    ...
    def changed(self, what):
        ...
        try:
            ...
            if hasattr(self.source, "event"):
                ...
                tn_log(f"[PARENTAL] تم جلب الحدث: {event.getEventName()}")
            elif hasattr(self.source, "getCurrentEvent"):
                event = self.source.getCurrentEvent()
                tn_log(f"[PARENTAL] تم جلب الحدث من getCurrentEvent: {event.getEventName()}")
            else:
            ...
            if os.path.exists(rating_file):
                tn_log(f"[PARENTAL] تم العثور على ملف التقييم: {rating_file}")
                with open(rating_file, "r") as f:
                    ...
                    if os.path.exists(fsk_path):
                        ...
                        self.instance.show()
                        tn_log(f"[PARENTAL] تم عرض FSK_{cert}.png")
                    else:
                        self.instance.hide()
