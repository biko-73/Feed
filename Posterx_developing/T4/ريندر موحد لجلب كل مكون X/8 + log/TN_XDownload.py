...
class TN_X_Downloader(threading.Thread):
        ...
        try:
            ...
            for res in data["results"]:
                ...
                if "images" in details:
                    ...
                    banners = [img for img in details["images"].get("posters", []) if "banner" in img.get("file_path", "").lower()]
                    if banners:
                ...
                with open(rating_file, "w", encoding="utf-8") as f:
                    f.write(rating)
                tn_log(f"[RATING] تم حفظ التقييم في: {rating_file}")
    ...
    def extract_rating(self, details, media_type):
        try:
            ...
            tn_log("[RATING] لم يُعثر على تصنيف، استخدام الافتراضي: PG")
            return "PG"
        except Exception as e:
            tn_log(f"[RATING] خطأ في استخراج التقييم: {e}")
            return "PG"
