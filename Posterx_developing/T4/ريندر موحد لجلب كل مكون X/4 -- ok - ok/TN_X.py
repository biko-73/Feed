...
class TN_X(Renderer):
    ...
    def changed(self, what):
        ...
        try:
            ...
            # 3. حاول من func() (مثل Event[1], Event[2])
            elif hasattr(self.source, "func"):
                try:
                    event = self.source.func()
                except:
                    pass
            # 4. إذا كان المصدر يحتوي على event بشكل مباشر
            elif isinstance(self.source, dict) and "event" in self.source:
                event = self.source["event"]
