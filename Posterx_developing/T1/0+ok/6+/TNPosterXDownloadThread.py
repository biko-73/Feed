...
# --- مفاتيح API ---
tmdb_api = "3c3efcf47c3577558812bb9d64019d65"  # تم التأكيد
thetvdb_api = "a99d487bb3426e5f3a60dea6d3d3c7ef"
fanart_api = "6d231536dea4318a88cb2520ce89473b"
omdb_api = "cb1d9f55"

# --- حجم الصورة ---
isz = "185,278"  # حجم البوستر
bsz = "640,360"  # حجم الباكدروب
...
class TNPosterXDownloadThread(threading.Thread):
	...
	def search_tmdb_backdrop(self, dwn_backdrop, title, shortdesc, fulldesc, channel=None):
		try:
			...
			resp = requests.get(url_tmdb, headers=headers, timeout=10).json()
			if resp.get("results"):
				# جلب معلومات الفيلم/العرض للحصول على الباكدروب
				movie_id = resp["results"][0].get("id")
				if movie_id:
					url_details = f"https://api.themoviedb.org/3/{srch}/{movie_id}?api_key={tmdb_api}&language={lang}"
	...
	def savePoster(self, dwn_poster, url_poster):
		try:
			r = requests.get(url_poster, stream=True, timeout=10, allow_redirects=True, headers=headers)
