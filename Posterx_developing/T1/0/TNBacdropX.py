...
# Based on digiteng, beber, sunriser
# Fixed & Optimized for Enigma2
...
class PosterDB(TNPosterXDownloadThread):
	...
	def run(self):
		while True:
			...
			if canal[6] == "backdrop":
				dwn_poster = path_folder + canal[5] + "_backdrop.jpg"
			if os.path.exists(dwn_poster):
				os.utime(dwn_poster, (time.time(), time.time()))
			else:
				if canal[6] == "backdrop":
					val, log = self.search_tmdb_backdrop(dwn_poster, canal[2], canal[4], canal[3], canal[0])
				else:
		...
		try:
			...
			if service:
				...
				if len(events) > self.nxts:
					...
					self.canal[5] = convtext(self.canal[2])
					# تحديد النوع
					if "backdrop" in str(self.skinAttributes):
						self.canal[6] = "backdrop"
					else:
						self.canal[6] = "poster"
			...
			curCanal = "{}-{}-{}".format(self.canal[1], self.canal[2], self.canal[6])
			if curCanal == self.oldCanal:
				return
			self.oldCanal = curCanal

			pstrNm = path_folder + self.canal[5] + ".jpg"
			if self.canal[6] == "backdrop":
				pstrNm = path_folder + self.canal[5] + "_backdrop.jpg"
