...
# Poster Downloader with AGP-level intelligence
			...
			if resp.get("results"):
				...
				if poster_path:
					...
					if self.savePoster(url_poster, dwn_poster):
						return True, f"[SUCCESS] TMDB: {title}"
...
def auto_clean_thread():
	while True:
		...
		try:
			from Components.Renderer.TNPosterX import path_folder
			clean_old_files(path_folder)
		except:
			pass
