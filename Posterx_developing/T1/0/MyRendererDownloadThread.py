# -*- coding: utf-8 -*-
# MyRendererDownloadThread.py
# Universal Graphics Downloader for MyRenderer
...
# --- استيراد المتغيرات من MyRenderer ---
try:
	from Components.Renderer.MyRenderer import poster_path, backdrop_path, banner_path, logo_path, convtext, apdb
except Exception as e:
	print(f"[MyRendererDownloadThread] Error importing from MyRenderer: {str(e)}")
	# تعريف افتراضي
	poster_path = "/tmp/MyRenderer/poster/"
	backdrop_path = "/tmp/MyRenderer/backdrop/"
	banner_path = "/tmp/MyRenderer/banner/"
	logo_path = "/tmp/MyRenderer/logo/"

# --- خيط التنزيل ---
class MyRendererDownloadThread(threading.Thread):
	def __init__(self):
...
# --- استيراد pdb من MyRenderer ---
try:
	from Components.Renderer.MyRenderer import pdb
except Exception as e:
	print(f"[MyRendererDownloadThread] Error importing pdb: {str(e)}")
	if PY3:
		import queue
		pdb = queue.LifoQueue()
	else:
...
# --- بدء تشغيل الخيط ---
threadDB = MyRendererDownloadThread()
...
def auto_clean_thread():
	while True:
		time.sleep(7200)
		try:
			from Components.Renderer.MyRenderer import poster_path, backdrop_path, banner_path, logo_path
			now = time()
