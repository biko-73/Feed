...
try:
    from .TN_PosterX import download_queue, tn_log
except:
    # تعريف احتياطي
    def tn_log(txt):
        try:
...
class TNPosterXDownloader(threading.Thread):
    ...
    def search_tvdb(self, title, desc, lang, filename):
        # TVDB يتطلب Authentication Token (ممكن إضافته لاحقًا)
        tn_log("[TVDB] غير مدعوم حاليًا (يتطلب token)")
        return False
