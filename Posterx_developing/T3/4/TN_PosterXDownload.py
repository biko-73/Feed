...
try:
    from .TN_PosterX import download_queue, tn_log, poster_folder, lng
    ...
    def tn_log(txt):
        try:
            with open("/media/hdd/logs/TN_PosterX.log", "a") as f:
...
class TNPosterXDownloader(threading.Thread):
    def __init__(self):
        ...
        self.daemon = True  # مهم: يسمح بالعمل في الخلفية
    ...
    def run(self):
        ...
        while True:
            try:
                # تقليل timeout لتفعيل التحقق بسرعة
                item = download_queue.get(timeout=5)
                tn_log(f"[QUEUE] معالجة: {item['title']}")
                self.download_poster(item)
                download_queue.task_done()
            except Empty:
                tn_log("[DOWNLOADER] لا يوجد مهام - الاستمرار...")
                continue
            except Exception as e:
                tn_log(f"[DOWNLOADER] خطأ عام: {e}")
                continue
    ...
    def download_poster(self, item):
        ...
        for src_name, src_func in sources:
            ...
            if src_func(title, short + " " + full, lang, filename):
                tn_log(f"[SUCCESS] تم التنزيل من {src_name}")
                return
