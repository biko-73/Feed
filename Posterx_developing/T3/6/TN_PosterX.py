...
# --- صف التنزيل (بحد أقصى 5 عناصر) ---
try:
    from queue import Queue
except:
    from Queue import Queue

download_queue = Queue(maxsize=5)  # ✅ تقليل الحجم
...
# --- قائمة بالطلبات المضافة (لمنع التكرار) ---
...
class TN_PosterX(Renderer):
    ...
    def changed(self, what):
        ...
        try:
            ...
            if self.nexts != 0:
                tn_log(f"[NEXTS] تجاهل الحدث {self.nexts} مؤقتًا")
                return

            # --- إضافة إلى الصف ---
            if download_queue.full():
                tn_log("[QUEUE] الصف ممتلئ - تخطي")
                return
    ...
    def check_and_show(self, retry):
        ...
        if os.path.exists(self.filename) and os.path.getsize(self.filename) > 0:
            ...
            if self.filename in pending_requests:
                pending_requests.remove(self.filename)
            self.timer.start(1, True)
        else:
            timer = eTimer()
