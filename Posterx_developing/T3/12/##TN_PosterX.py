...
class TN_PosterX(Renderer):
    def __init__(self):
        ...
        # أضف الريندر إلى القائمة
        if self not in active_renderers:
            active_renderers.append(self)
        tn_log(f"[INIT] TN_PosterX تم تهيئة (nexts={self.nexts})")
    ...
    def changed(self, what):
        ...
        # --- معالجة كل الريندرز عند تغيير أي حدث ---
        if what[0] == 1:  # حدث تغير
            self.schedule_update_all()

    def schedule_update_all(self):
        """تحديث جميع الريندرز بعد تغيير الحدث"""
        timer = eTimer()
        timer.callback.append(self.update_all_renderers)
        timer.start(1, True)  # مؤجل قليلاً

    def update_all_renderers(self):
        for renderer in active_renderers:
            renderer.fetch_poster()

    def fetch_poster(self):
        try:
            event = self.source.event
            if not event:
                tn_log(f"[EVENT] لا يوجد حدث (nexts={self.nexts})")
                self.instance.hide()
                return
            ...
            if event_id == self.current_event_id:
                tn_log(f"[DUPLICATE] نفس الحدث - تخطي (nexts={self.nexts})")
                return
            self.current_event_id = event_id

            clean_name = clean_title(title)
            if not clean_name:
                tn_log(f"[CLEAN] الاسم فارغ (nexts={self.nexts})")
                self.instance.hide()
                return

            self.filename = os.path.join(poster_folder, f"{clean_name}.jpg")
            tn_log(f"[FILE] المسار: {self.filename} (nexts={self.nexts})")
            ...
            if os.path.exists(self.filename) and os.path.getsize(self.filename) > 0:
                tn_log(f"[CACHE] عرض من الكاش: {self.filename} (nexts={self.nexts})")
                self.timer.start(1, True)
                return

            # --- منع التكرار ---
            if self.filename in pending_requests:
                tn_log(f"[PENDING] طلب مكرر - تخطي: {clean_name} (nexts={self.nexts})")
                return
            ...
            tn_log(f"[QUEUE] إضافة للتنزيل: {title} (nexts={self.nexts})")
