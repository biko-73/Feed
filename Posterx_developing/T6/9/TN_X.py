...
class TN_X(Renderer):
    ...
    def changed(self, what):
        ...
        try:
            ...
            # فقط اعتمد على eEPGCache عندما يكون nextEvent > 0
            if self.nextEvent > 0 and hasattr(self.source, "service"):
                try:
                    ...
                    if ref:
                        ...
                        if events and len(events) > self.nextEvent:
                            ...
                            tn_log(f"[EPGCACHE] لا يوجد حدث برقم {self.nextEvent}")
                except Exception as e:
                    tn_log(f"[EPGCACHE] خطأ: {e}")

            # إذا لم ينجح eEPGCache، أو nextEvent=0، استخدم self.source.event
            if not event and hasattr(self.source, "event") and self.source.event:
