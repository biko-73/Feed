...
omdb_api = "cb1d9f55"  # المفتاح الذي قدمته
...
class TN_X_Downloader(threading.Thread):
    ...
    def download_all(self, item):
        ...
        # ثانيًا: حاول جلب من OMDB
        result_omdb = self.search_omdb(title, clean_name, rating_file, score_file)
        if result_omdb:
            tn_log(f"[SUCCESS] تم التنزيل من OMDB")
            self.remove_from_pending(poster_file)
            if self.on_download_complete:
                self.on_download_complete(clean_name)
            return
    ...
    def search_omdb(self, title, clean_name, rating_file, score_file):
        try:
            query = requests.utils.quote(title)
            ...
            tn_log(f"[OMDB] التقييم: {rated}, النجوم: {imdb_rating}")

            with open(rating_file, "w", encoding="utf-8") as f:
                f.write(rated)

            with open(score_file, "w") as f:
                f.write(imdb_rating if imdb_rating != "N/A" else "0")
