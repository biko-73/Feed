...
try:
    from .TN_XTools import clean_title, auto_update_mapping
except Exception as e:
    print(f"[ERROR] فشل تحميل TN_XTools: {e}")
...
try:
    from .TN_title_mapping import TITLE_MAPPING, NO_IMAGE_NAMES
except:
    TITLE_MAPPING = {}
    NO_IMAGE_NAMES = []
...
class TN_X_Downloader(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                tn_log(f"[QUEUE] معالجة: {item['title']} (type: {item.get('type', 'poster')})")
    ...
    def download_all(self, item):
        ...
        title_mapping, no_image_names = {}, []
        try:
            from .TN_XTools import load_mapping
            title_mapping, no_image_names = load_mapping()
        except:
            pass

        if title.strip() in no_image_names:
            tn_log(f"[SKIP] تجاهل اسم تقني: {title}")
            return
        ...
        for lang in langs:
            ...
            if result:
                ...
                # ✅ التحديث التلقائي للخريطة
                auto_update_mapping(item["title"], clean_name)
                request_key = (clean_name, item.get("type", "poster"))
                if request_key in self.pending_requests:
                    self.pending_requests.remove(request_key)
        ...
        try:
            ...
            for res in data["results"]:
                ...
                if not os.path.exists(banner_file):
                    self.download_banner_fanart(media_type, tmdb_id, banner_file)
    ...
    def get_details(self, media_type, tmdb_id, lang):
        try:
            url = f"https://api.themoviedb.org/3/{media_type}/{tmdb_id}?api_key={tmdb_api}&language={lang}&append_to_response=images,content_ratings,release_dates"
    ...
    def download_banner_fanart(self, media_type, tmdb_id, banner_file):
        try:
            show_type = "movies" if media_type == "movie" else "tv"
            url = f"http://webservice.fanart.tv/v3/{show_type}/{tmdb_id}?api_key={fanart_api}"
            tn_log(f"[FANART] جلب بانر من: {url}")
            resp = self.session.get(url, timeout=10)
            if resp.status_code != 200:
                return False
            data = resp.json()

            banners = data.get("tvthumb" if media_type == "tv" else "moviebanner", [])
            arabic_banners = [b for b in banners if b.get("lang") == "ar"]
            banner_list = arabic_banners or banners
            if banner_list:
                best_banner = sorted(banner_list, key=lambda x: x.get("likes", 0), reverse=True)[0]
                image_url = best_banner["url"]
                tn_log(f"[FANART] تنزيل بانر: {image_url}")
                self.download_image(image_url, banner_file)
                return True
            return False
        except Exception as e:
