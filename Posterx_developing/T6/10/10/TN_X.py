...
class TN_X(Renderer):
    ...
    def get_search_languages(self, title):
        if re.search(r'[\u0600-\u06FF]', title):  # عربي
            ...
            return ['tr', 'en']
        elif re.search(r'[šđčćž]', title):  # صربي
            return ['cs', 'en']
        else:
            return ['en']
    ...
    def changed(self, what):
        ...
        try:
            ...
            # 1. دعم ServiceEvent مع eEPGCache (للأحداث التالية)
            ...
            # 2. دعم self.source.event (للحدث الحالي فقط)
            elif hasattr(self.source, "event") and self.source.event:
            ...
            # ✅ المفتاح: (clean_name, type)
            request_key = (self.clean_name, self.type)
            if request_key in pending_requests:
                tn_log(f"[PENDING] طلب مكرر - تخطي: {self.clean_name} (type: {self.type})")
                return
            ...
            pending_requests.add(request_key)
    ...
    def showContent(self):
        try:
            if self.instance and os.path.exists(self.filename) and os.path.getsize(self.filename) > 0:
