...
class TN_X(Renderer):
    ...
    def showPicture(self):
        try:
            # أولاً: جرب self.source.service (لـ nextEvent > 0)
            if hasattr(self.source, "service") and self.source.service:
                ...
                if events and len(events) > self.nextEvent:
                    ...
                    self.download_and_show()
                    return

            # ثانيًا: إذا فشل، استخدم self.source.event (للحدث الحالي)
            elif hasattr(self.source, "event") and self.source.event:
                event = self.source.event
                title = event.getEventName()
                self.clean_name = clean_title(title)
                tn_log(f"[SOURCE] تم جلب الحدث من self.source.event: {title}")
                self.download_and_show()
                return
