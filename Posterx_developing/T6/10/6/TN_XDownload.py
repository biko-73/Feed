...
class TN_X_Downloader(threading.Thread):
    ...
    def download_all(self, item):
        ...
        if recent:
            self.remove_from_pending(clean_name)
            return

        for lang in langs:
            result = self.search_tmdb(title, clean_name, lang, poster_file, backdrop_file, logo_file, banner_file, rating_file, score_file)
            if result:
    ...
    def remove_from_pending(self, clean_name):
        if clean_name in self.pending_requests:
            self.pending_requests.remove(clean_name)

    def search_tmdb(self, title, clean_name, lang, poster_file, backdrop_file, logo_file, banner_file, rating_file, score_file):
        try:
        ...
        try:
            if media_type == "tv":
                for r in details.get("content_ratings", {}).get("results", []):
                    if r.get("iso_3166_1") in ["US", "GB", "AE"]:
