...
class TN_XParental(Renderer):
    ...
    def changed(self, what):
        ...
        if not self.instance or not self.instance.valid():
            return
        ...
        try:
            ...
            if os.path.exists(rating_file):
                with open(rating_file, "r") as f:
                    ...
                    if os.path.exists(fsk_path) and self.instance.valid():
                        self.instance.setPixmapFromFile(fsk_path)
                        self.instance.show()
                    else:
                        self.instance.hide()
                        tn_log(f"[PARENTAL] الصورة غير موجودة أو instance ميت: {fsk_path}")
            else:
                self.instance.hide()
