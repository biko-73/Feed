...
#check image name
imgnm=$(curl -s "http://images.mynonpublic.com/openatv/7.4/index.php?open=$device" | grep -o "$device/.*\.zip" | tail -n 1 | sed "s/$device\///" | cut -d '>' -f1 | tr -d "'" )
...
wget -O $ms/images/$imgnm http://images.mynonpublic.com/openatv/7.4/current.php?open=$device
